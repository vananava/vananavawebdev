...
if ($_SERVER['REMOTE_ADDR'] == $testip) { 
	$track1	=	json_encode($_REQUEST, JSON_UNESCAPED_UNICODE);

	$sql	=	"
		INSERT INTO vananava_wp_vnnv_track_data (track1,track2,track3,add_date)
		VALUES ('$track1','','',NOW())
	";
	$query = mysql_query($sql) or die("Can't query");

	$track_data_id = mysql_insert_id();
...
foreach($ItemCodeMail as $value) {
	...
	if ($_SERVER['REMOTE_ADDR'] == $testip) { 
		$track2	=	json_encode($itemsMail, JSON_UNESCAPED_UNICODE);
		$sql	=	"
			UPDATE  vananava_wp_vnnv_track_data SET
				track2	=	'$track2'
			WHERE track_data_id = '$track_data_id'
		";
		$query = mysql_query($sql) or die("Can't query");
	}
	...
	if($ItemCodeMail[$i]==1184 || $ItemCodeMail[$i]==1185 ){
		...
		if ($_SERVER['REMOTE_ADDR'] == $testip) { 
			$track3	=	"quan-".$QuantityMail[$i];
			$sql	=	"
				UPDATE  vananava_wp_vnnv_track_data SET
					track3	=	CONCAT(track3,'$track3')
				WHERE track_data_id = '$track_data_id'
			";
			$query = mysql_query($sql) or die("Can't query");
		}
		...
		if ($_SERVER['REMOTE_ADDR'] == $testip) { 
			...
			$TotalPaidMail+=100.0*$QuantityMail[$i];
		}
		

		if ($_SERVER['REMOTE_ADDR'] == $testip) { 
			$track3	=	"totalpaid".$TotalPaidMail;
			$sql	=	"
				UPDATE  vananava_wp_vnnv_track_data SET
					track3	=	CONCAT(track3,'$track3')
				WHERE track_data_id = '$track_data_id'
...
if ($_SERVER['REMOTE_ADDR'] == $testip) { 
	/*$track2	=	json_encode($items, JSON_UNESCAPED_UNICODE);
	$track3	=	json_encode($itemsMail, JSON_UNESCAPED_UNICODE);
...
if ($_SERVER['REMOTE_ADDR'] == $testip) { 
	/*$track2	=	json_encode($json_data, JSON_UNESCAPED_UNICODE);
	$track3	=	json_encode($json_data_mail, JSON_UNESCAPED_UNICODE);
